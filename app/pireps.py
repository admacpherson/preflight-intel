...
def fetch_pireps_for_route(origin_coords: tuple, destination_coords: tuple, lookback_hours: int) -> list[dict]:
    ...
    for i in range(num_segments):
        ...
        seg_origin = (segment_points[0][1], segment_points[0][0])       # (lat, lon)
        seg_dest = (segment_points[-1][1], segment_points[-1][0])

        bbox = compute_bbox(seg_origin, seg_dest, padding_deg=1.0)
        segment_pireps = fetch_pireps(bbox, lookback_hours=lookback_hours)

        # Deduplicate by receipt time + icao id
